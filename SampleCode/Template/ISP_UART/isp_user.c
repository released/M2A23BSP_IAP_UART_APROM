...
int ParseCmd(unsigned char *buffer, uint8_t len)
{
    ...
    pSrc += 8;
    srclen -= 8;
    ReadData(Config0, Config0 + 8, (unsigned int *)(response + 8)); //read config
    ...
    else if((lcmd == CMD_UPDATE_APROM) || (lcmd == CMD_ERASE_ALL))
    {
        ...
        EraseAP(APROM_APPLICATION_START, g_apromSize);
